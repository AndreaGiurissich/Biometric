...
in between. The CMC curves (Fig.~\ref{fig:cmc} for SIFT) confirm that the correct
identity is retrieved within the very top ranks even at Hard.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.60\textwidth]{cmc_sift.png}
\caption{CMC curve for SIFT (baseline). The recognition rate reaches near-100\%
within a few ranks at every level. (CMC curves for Gabor and DINOv2 are added once
those models are re-run with per-probe archiving.)}
\label{fig:cmc}
\end{figure}
...
($+0.7$/$+3.0$/$+3.4$ Rank-1 points); is \textbf{marginal for the deep embedding}
(DINOv2, $\sim\!+0.6$ flat); and is \textbf{statistically neutral for the keypoint
matcher (SIFT)} --- the paired McNemar test is non-significant at every level
(Easy $p=0.69$, Medium $p=0.23$, Hard $p=0.15$), and sharpening even slightly hurts
SIFT at Hard, plausibly by introducing spurious keypoints that break geometric
consistency. A preprocessing operator devised for facial robustness is therefore
not a universal good: its value depends on the downstream representation. (The
paired significance for Gabor and DINOv2 is computed once those models are
re-run with per-probe archiving; their $\Delta$ values here are point estimates.)

\begin{table}[h]
\centering
\caption{$\Delta$Rank-1 from preprocessing (percentage points). For SIFT the paired
McNemar test is non-significant (ns) at every level.}
\label{tab:prep}
\begin{tabular}{@{}lccc@{}}
\toprule
